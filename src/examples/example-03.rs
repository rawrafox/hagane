...
impl RSMRenderer for Example03Renderer {
  fn initialize(&mut self, view: RSMViewID) {
    ...
    view.set_depth_stencil_pixel_format(252); // MTLPixelFormatDepth32Float
    ...
    depth_stencil_descriptor.set_depth_compare_function(MTLCompareFunction::MTLCompareFunctionLess);
  ...
  fn draw(&mut self, view: RSMViewID) {
    ...
    command_encoder.set_front_facing_winding(MTLWinding::MTLWindingCounterClockwise);
    command_encoder.set_cull_mode(MTLCullMode::MTLCullModeBack);
    ...
    command_encoder.draw_indexed_primitives_index_count_index_type_index_buffer_index_buffer_offset(MTLPrimitiveType::MTLPrimitiveTypeTriangle, INDICES.len(), MTLIndexType::MTLIndexTypeUInt16, self.index_buffer.clone(), 0);
